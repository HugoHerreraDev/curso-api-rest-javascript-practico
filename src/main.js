...
async function getMoviesBySearch(query){
    ...
    createMovies(movies, genericSection, {lazyLoad: false, clean: true});
}
...
async function getMovieById(id){
    const { data: movie } = await api(`movie/${id}`); //data se recibe de axios y se renombra a: movie
    ...
    movieDetailScore.innerHTML = movie.vote_average.toFixed(1); //toFixed(1) es para que redondee a 1 decimal
...
async function getRelatedMoviesById(id){
    ...
    createMovies(relatedMovies, relatedMoviesContainer, {lazyLoad: false, clean: true});
}
